...
namespace UnitTesting.ServerTest
{
    ...
    public class Server_test
    {
        ...
        {
            string dbDir = @"C:\Connect Four\Test 2";
            if (!Directory.Exists(dbDir))
            {
                var dbProps = new DatabaseProperties(dbDir, 7, 6);
            ...
            using (var db = new Database(dbDir))
            {
                ...
                int times = 0;
                int dbLength = db.getDatabaseLength();

                int gameCount = 10000;
                int batchCount = 1;
                var iterationTime = new TimeSpan();

                while (times < 5)//DateTime.Now.Add(iterationTime) < new DateTime(2015, 12, 12, 18, 10, 0))
                {
                    sw.Restart();

                    int aliceWon = 0;
                    int bobWon = 0;
                    var histories = new List<byte>();
                    ...
                    for (int j = 0; j < batchCount; j++)
                    {
                        for (int i = 0; i < gameCount; i++)
                        {
                            ...
                            while (g.has_won(players.Empty) && g.stones_count != 42)
                            {
                                ...
                                {
                                    column = db.get_column(g.get_field());
                                }
                                else
                                {
                            ...
                            {
                                aliceWon++;
                                data[0] = Network_codes.game_history_alice;
                            }
                            else
                            {
                                bobWon++;
                                data[0] = Network_codes.game_history_bob;
                            }

                            data[0] = g.has_won(players.Alice) ? Network_codes.game_history_alice : Network_codes.game_history_bob;
                            Buffer.BlockCopy(g.history, 0, data, 1, g.stones_count);
                            histories.AddRange(data);
                        }
                    }
                    ...
                    int fieldsProc = db.preprocess_game_history(histories.ToArray());

                    var processingTime = sw.Elapsed;
                    iterationTime = simulationTime + processingTime;

                    times++;
                    int newLength = db.getDatabaseLength();
